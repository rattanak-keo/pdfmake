...
PdfPrinter.prototype.createPdfKitDocument = function (docDefinition, options) {
	...
	docDefinition.tagged =
		typeof docDefinition.tagged === "boolean" ? docDefinition.tagged : false;
	docDefinition.displayTitle =
		typeof docDefinition.displayTitle === "boolean"
			? docDefinition.displayTitle
			: false;
	docDefinition.compress = isBoolean(docDefinition.compress)
		? docDefinition.compress
		: true;
	docDefinition.images = docDefinition.images || {};
	docDefinition.pageMargins =
		docDefinition.pageMargins !== undefined &&
		docDefinition.pageMargins !== null
			? docDefinition.pageMargins
			: 40;
	...
	var pdfOptions = {
		...
		fontLayoutCache: isBoolean(options.fontLayoutCache)
	...
	var builder = new LayoutBuilder(
		pageSize,
		fixPageMargins(docDefinition.pageMargins),
...
function renderLine(line, x, y, patterns, pdfKitDoc) {
	...
	for (var i = 0, l = line.inlines.length; i < l; i++) {
		...
		// Store custom decoration thickness and spacing for each inline
		if (inline.decorationStyle) {
			if (inline.decorationStyle.thickness) {
				inline._decorationThickness = inline.decorationStyle.thickness;
			}
			if (inline.decorationStyle.spacing) {
				inline._decorationSpacing = inline.decorationStyle.spacing;
			}
		}
	...
	// Create decorationOptions object
	var decorationOptions = {};

	// Check if any inline has custom thickness or spacing
	var hasCustomDecorations = line.inlines.some(function (inline) {
		return inline._decorationThickness || inline._decorationSpacing;
	});

	if (hasCustomDecorations) {
		decorationOptions.customDecorations = true;
		decorationOptions.thicknessMap = {};
		decorationOptions.spacingMap = {};
		for (var j = 0; j < line.inlines.length; j++) {
			if (line.inlines[j]._decorationThickness) {
				decorationOptions.thicknessMap[j] = line.inlines[j]._decorationThickness;
			}
			if (line.inlines[j]._decorationSpacing) {
				decorationOptions.spacingMap[j] = line.inlines[j]._decorationSpacing;
			}
		}
	}

	// Decorations won't draw correctly for superscript
	textDecorator.drawDecorations(line, x, y, pdfKitDoc, decorationOptions);
}
	...
	switch (vector.type) {
		case "ellipse":
			...
			if (vector.linearGradient) {
				gradient = pdfKitDoc.linearGradient(
					vector.x - vector.r1,
					vector.y,
					vector.x + vector.r1,
					vector.y
...
function renderSVG(svg, x, y, pdfKitDoc, fontProvider) {
	...
	if (svg.linkToPage) {
		pdfKitDoc
			.ref({ Type: "Action", S: "GoTo", D: [svg.linkToPage, 0, 0] })
			.end();
		pdfKitDoc.annotate(svg.x, svg.y, svg._width, svg._height, {
			Subtype: "Link",
